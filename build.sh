...
if [ -z "$MYSQL_DATABASE" ] && [ -z "$POSTGRES_DATABASE" ] && [ "${MYSQL_DATABASE,,}" != "true" ] && [ "${POSTGRES_DATABASE,,}" != "true" ]; then
    ...
    python manage.py createsuperuser createsuperuser --college_id=0 --email=aryanamish385@gmail.comm --role=4 --noinput
    python manage.py createsuperuser createsuperuser --college_id=1 --email=aryanamish385@gmail.comm --role=4 --noinput
else
