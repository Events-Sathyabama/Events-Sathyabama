...
class AxiosInstance {
  ...
  async login(username: string, password: string) {
    if (typeof window !== 'undefined') {
      const request = await this.post(get_url('login'), {college_id: username, password: password});
  ...
  __check_expiry() {
    ...
    if(token !== null){
      ...
      return expiryTime !== null && parseInt(expiryTime) >= parseInt(String((new Date().getTime()) / 1000))
    }
    return false;
  }
}
